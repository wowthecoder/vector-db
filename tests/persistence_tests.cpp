...
TEST(PersistenceTest, LoadRejectsWrongMagicBytes)
{
    ...
    overwrite_bytes(file.path(), k_magic_offset, std::string{'N', 'O', 'T', 'V', 'D', 'B', '\0', '\0'});
...
TEST(PersistenceTest, LoadRejectsUnsupportedVersion)
{
    ...
    const vectordb::Collection collection(2, vectordb::Metric::L2);
    collection.save(file.path());
    overwrite_bytes(file.path(), k_version_offset, std::string{'\x02', '\0', '\0', '\0'});
...
TEST(PersistenceTest, LoadRejectsInvalidMetricCode)
{
    ...
    const vectordb::Collection collection(2, vectordb::Metric::L2);
    collection.save(file.path());
    overwrite_bytes(file.path(), k_metric_offset, std::string{'\x63', '\0', '\0', '\0'});
...
TEST(PersistenceTest, LoadRejectsDuplicateExternalIds)
{
    ...
    const std::streamoff second_id_offset = k_first_record_offset + first_record_size + 4;
    overwrite_bytes(file.path(), second_id_offset, "alpha");

    EXPECT_THROW(vectordb::Collection::load(file.path()), std::invalid_argument);
}
...
TEST(PersistenceTest, LoadRejectsEmptyExternalId)
{
    ...
    collection.save(file.path());
    overwrite_bytes(file.path(), k_first_record_offset, std::string{'\0', '\0', '\0', '\0'});
