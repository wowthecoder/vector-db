...
namespace vectordb
{
    namespace
    {
        ...
        bool is_better_result(const InternalSearchResult &a, const InternalSearchResult &b, bool higher_is_better)
        {
            if (a.score == b.score)
            {
        ...
        {
            const auto better = [higher_is_better](const InternalSearchResult &a, const InternalSearchResult &b)
            {
    ...
    std::vector<InternalSearchResult> FlatIndex::search(std::span<const float> query, std::size_t top_k) const
    {
        ...
        for (std::uint64_t internal_id = 0; internal_id < vectors_.size(); ++internal_id)
        {
