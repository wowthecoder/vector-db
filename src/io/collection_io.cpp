...
namespace
{
    constexpr std::array<char, 8> k_magic_bytes{'V', 'D', 'B', 'C', 'O', 'L', 'L', '\0'};
    ...
    */
    ...
    std::uint32_t read_u32(std::istream &in)
    {
        ...
        for (std::size_t i = 0; i < bytes.size(); ++i)
        {
            value |= static_cast<std::uint32_t>(static_cast<unsigned char>(bytes[i])) << (i * 8);
        }
    ...
    std::uint64_t read_u64(std::istream &in)
    {
        ...
        for (std::size_t i = 0; i < bytes.size(); ++i)
        {
            value |= static_cast<std::uint64_t>(static_cast<unsigned char>(bytes[i])) << (i * 8);
        }
...
namespace vectordb
{
    void Collection::save(const std::filesystem::path &path) const
    {
        ...
        {
            const std::string &external_id = internal_to_external_.at(static_cast<std::size_t>(internal_id));
            ...
            write_exact(out, reinterpret_cast<const char *>(vector), dim() * sizeof(float));
        }
    }
    ...
    std::unique_ptr<Collection> Collection::load(const std::filesystem::path &path)
    {
        ...
        {
            throw std::runtime_error("Unsupported collection file version: " + std::to_string(version));
        }
        ...
        case 2:
            ...
            break;
        default:
            throw std::runtime_error("Invalid Metric code, should be either 0, 1 or 2");
        }
        ...
        for (std::uint64_t internal_id = 0; internal_id < count; ++internal_id)
        {
            ...
            read_exact(in, reinterpret_cast<char *>(values.data()), dimension * sizeof(float));
